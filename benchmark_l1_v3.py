"""Benchmark L1 v3: Fragment-level compression + v3 improvements.

Tests v3 improvements:
  - Helper-prefix demotion (帮我写→写)
  - Transition word removal (很好, 太好了, 接下来)
  - Redundant modifier stripping (完整的, 功能齐全的)
  - Trailing particle cleanup (了, 吧, 呢)
  - Redundant quantifier stripping (写一个X→写X)

Run: python benchmark_l1_v3.py
"""
...
def run_benchmark():
    print("=" * 70)
    print("L1 v3 Benchmark -- v3 Compression")
    print("=" * 70)
    ...
    print(f"{'=' * 70}")
    print(f"  v1 (MODERATE, 6 scenarios):  ~1367 -> ~1192 = 12.8% savings")
    print(f"  v2 (MODERATE, 6 scenarios):  ~1692 -> ~1295 = 23.5% savings")
    for r in all_results:
        print(f"  v3 ({r['level']:<10}, 8 scenarios):  ~{r['total_original']} -> ~{r['total_compressed']} = {r['total_savings_pct']}% savings")
    ...
    for r in all_results:
        ...
        print(f"  v3+L0 ({r['level']:<10}): ${cost:.6f} (save {pct}%)")

    results_path = os.path.join(os.path.dirname(__file__), "l1_benchmark_v3_results.json")
